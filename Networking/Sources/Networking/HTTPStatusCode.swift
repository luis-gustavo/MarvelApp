...
/**
 ...
 The first digit of the status code specifies one of five classes of response; an HTTP
 client must recognise these five classes at a minimum.
 **/
public enum HTTPStatusCode: Int, Error {
    ...
    enum ResponseType {
        ...
        /// - success: This class of status codes indicates the action requested by the
        /// client was received, understood, accepted, and processed successfully
        /// .
        case success

        /// - redirection: This class of status code indicates the client must
    ...
    /// - processing: This code indicates that the server has received and is
    ...
    ///  received a 200 OK from its origin, but is returning a modified version of the origin's response.
    ...
    /// - multiStatus: The message body that follows is an XML message and can
    /// contain a number of separate response codes, depending on how many sub-requests were made.
    case multiStatus = 207

    /// - alreadyReported: The members of a DAV binding have already been
    /// enumerated in a previous reply to this request, and are not being included again.
    case alreadyReported = 208

    /// - IMUsed: The server has fulfilled a request for the resource, and the response
    ...
    /// - methodNotAllowed: A request method is not supported for the requested
    /// resource. e.g. a GET request on a form which requires data to be presented via POST
    ...
    /// - conflict: Indicates that the request could not be processed because of conflict
    ...
    /// - lengthRequired: The request did not specify the length
    ...
    /// - rangeNotSatisfiable: The client has asked for a portion
    /// of the file (byte serving), but the server cannot supply that portion.
    ...
    /// - upgradeRequired: The client should switch to a different
    ...
    /// - requestHeaderFieldsTooLarge: The server is unwilling
    ///  to process the request because either an individual header field,
    ///   or all the header fields collectively, are too large.
    case requestHeaderFieldsTooLarge = 431

    /// - noResponse: Used to indicate that the server has returned
    /// no information to the client and closed the connection.
    case noResponse = 444

    /// - unavailableForLegalReasons: A server operator has received
    /// a legal demand to deny access to a resource or to a set of resources that includes the requested resource.
    case unavailableForLegalReasons = 451

    /// - SSLCertificateError: An expansion of the 400 Bad Request
    /// response code, used when the client has provided an invalid client certificate.
    case SSLCertificateError = 495

    /// - SSLCertificateRequired: An expansion of the 400 Bad
    /// Request response code, used when a client certificate is required but not provided.
    case SSLCertificateRequired = 496

    /// - HTTPRequestSentToHTTPSPort: An expansion of the 400
    /// Bad Request response code, used when the client has made a HTTP request to a port listening for HTTPS requests.
    ...
    /// - gatewayTimeout: The server was acting as a gateway or proxy and
